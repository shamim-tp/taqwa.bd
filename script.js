...
document.addEventListener('DOMContentLoaded', async function() {
  try {
    ...
    initLoginTabs();
    enhanceMobileMenu();  // উন্নত মোবাইল মেনু (ওভারলে সহ)
    renderSidebarNav();   // সাইডবার নেভিগেশন তৈরি
...
// Utility functions (already defined, but we keep them)
...
// Enhanced toast with type support
...
function renderSidebarNav() {
  ...
  nav.innerHTML = items.map(item => `
    <button class="nav-item" onclick="navigateTo('${item.page}')">
      <span>${item.icon}</span>
...
window.navigateTo = function(page) {
  ...
  // পৃষ্ঠা অনুযায়ী টাইটেল ও কন্টেন্ট সেট (ডেমো)
...
// মোবাইল মেনু উন্নয়ন (ওভারলে ও টাচ) – HTML এর active ক্লাস ব্যবহার করে
function enhanceMobileMenu() {
  ...
  function toggleMenu(force) {
    const willShow = force !== undefined ? force : !sidebar.classList.contains('active');
    if (willShow) {
  ...
  // সাইডবারের ভেতরের বাটনে ক্লিক করলে মোবাইলে মেনু বন্ধ
  ...
  // উইন্ডো রিসাইজ করলে মেনু রিসেট
  window.addEventListener('resize', () => {
    if (window.innerWidth > 768) toggleMenu(false);
  });

  // Escape কী প্রেস করলে মেনু বন্ধ
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && sidebar.classList.contains('active')) toggleMenu(false);
  });
}
...
window.logout = logout; // make logout available globally
